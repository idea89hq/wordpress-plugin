...
| **WooCommerce-native add-to-cart** | Adds products (including variable-product variations) to the real WooCommerce cart via the Store API — no custom cart, no theme override |
...
The widget script is served from the IDEA89 CDN — no static widget assets are bundled into this plugin.

## Error handling

The plugin never breaks the storefront:

- Every API call goes through a single client that catches WordPress HTTP errors, checks the response code, and returns a plain boolean or shaped array. Failures are logged (behind `WP_DEBUG`) and swallowed — nothing propagates to a page render.
...
- With no API key configured, every sync path is a no-op — the plugin makes zero external HTTP calls.
